...
interface ActivityItem {
  id: string;
  type: 'sensor_added' | 'sensor_expired' | 'sensor_issue' | 'achievement' | 'upcoming_expiry';
  title: string;
  description: string;
  timestamp: Date;
...
export function ActivityTimeline({ sensors, userAchievements = [] }: ActivityTimelineProps) {
  const { user } = useAuth();
  ...
  const generateActivities = () => {
    ...
    recentSensors.forEach(sensor => {
      activityItems.push({
        ...
        description: `${sensor.sensor_models?.manufacturer || 'Unknown'} ${sensor.sensor_models?.model_name || 'sensor'} - ${sensor.serial_number}`,
    ...
    // Problematic sensors
    const problematicSensors = sensors.filter(sensor => sensor.is_problematic);
    ...
    upcomingExpirations.forEach(sensor => {
      ...
      activityItems.push({
        ...
        description: `${sensor.serial_number} expires in ${hoursLeft < 24 ? `${hoursLeft} hours` : `${Math.ceil(hoursLeft / 24)} days`}`,
      ...
      <div className="bg-[#1e293b] rounded-lg p-6 border border-slate-700/30">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-slate-400">Your sensor tracking timeline</p>
          </div>
        </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-slate-400 mb-2">No recent activity</p>
