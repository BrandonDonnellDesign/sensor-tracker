...
export const DEFAULT_DASHBOARD_WIDGETS: DashboardWidget[] = [
  ...
  // Glucose - MOVED UP for priority

  { id: 'glucose-chart', type: 'glucose-chart', title: 'Glucose Trends', category: 'glucose', enabled: true, size: 'full', order: 2 },



  { id: 'sensor-alerts', type: 'sensor-alerts', title: 'Sensor Alerts', category: 'sensors', enabled: true, size: 'full', order: 3 },

  // Insulin
  { id: 'iob-tracker', type: 'iob-tracker', title: 'IOB Tracker', category: 'insulin', enabled: true, size: 'small', order: 4 },
  { id: 'quick-dose', type: 'quick-dose', title: 'Quick Dose Logger', category: 'insulin', enabled: false, size: 'small', order: 5 },
  { id: 'tdi', type: 'tdi', title: 'Total Daily Insulin', category: 'insulin', enabled: false, size: 'large', order: 6 },
  { id: 'basal-trends', type: 'basal-trends', title: 'Basal Trends', category: 'insulin', enabled: false, size: 'large', order: 7 },

  // Activity & Insights
  { id: 'ai-insights', type: 'ai-insights', title: 'AI Insights', category: 'general', enabled: true, size: 'large', order: 8 },
  { id: 'activity-timeline', type: 'activity-timeline', title: 'Activity Timeline', category: 'general', enabled: true, size: 'large', order: 9 },
    ...
    <div className={`bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 ${className}`}>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === 'sensors'
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === 'insulin'
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === 'glucose'
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === 'food'
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCategory === 'general'
                ...
                <Draggable key={widget.id} draggableId={widget.id} index={index}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`bg-gray-50 dark:bg-slate-700 rounded-lg p-4 border border-gray-200 dark:border-slate-600 ${snapshot.isDragging ? 'shadow-lg' : ''
                        }`}
                    >
                        ...
                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <h4 className={`font-medium truncate ${widget.enabled
                          ...
                          <div className="flex space-x-1">
                            {(['small', 'medium', 'large', 'full'] as const).map((size) => (
                              <button
                                ...
                                className={`px-2 py-1 text-xs rounded transition-colors ${widget.size === size
                                    ? 'bg-indigo-600 text-white'
                      ...
                      <div className="mt-3 grid grid-cols-4 gap-1 h-8">
                        {Array.from({ length: 4 }).map((_, i) => (
                          <div
                            key={i}
                            className={`rounded ${(widget.size === 'small' && i === 0) ||
        ...
        <div className="grid grid-cols-4 gap-2 h-32 bg-gray-100 dark:bg-slate-700 rounded-lg p-3 overflow-auto">
          {localWidgets
            ...
            .map((widget) => (
              <div
                key={widget.id}
                className={`rounded text-xs p-1 flex items-center justify-center text-center ${CATEGORY_COLORS[widget.category]
