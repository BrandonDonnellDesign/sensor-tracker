...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Placeholder response - sensors table will be created in future migration
    const sensors: any[] = [];
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        apiVersion: '1.0.0',
        note: 'Sensor management will be available after database migration'
      }
    });
  ...
  try {
    ...
    return NextResponse.json(
      { 
        ...
        message: 'Sensor management will be available after database migration' 
      },
      { status: 501 }
    );
    ...
    console.error('Error in sensors POST:', error);
    return NextResponse.json(
      { error: 'internal_error', message: 'Internal server error' },
      { status: 500 }
    );
