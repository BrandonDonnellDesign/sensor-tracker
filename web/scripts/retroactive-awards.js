...
async function awardAllUsers() {
  ...
  try {
    const { data, error } = await supabase.rpc('retroactively_award_achievements');
...
async function awardSpecificUser(userId) {
  console.log(`🏆 Starting retroactive achievement awards for user: ${userId}\n`);
...
async function checkGamificationStats() {
  ...
  try {
    ...
    if (!stats || stats.length === 0) {
      console.log('ℹ️  No gamification data found. Run retroactive awards first.');
      return;
    }
    ...
    const averageLevel = (stats.reduce((sum, s) => sum + s.level, 0) / totalUsers).toFixed(1);
    ...
    topUsers.forEach((user, index) => {
      console.log(`   ${index + 1}. ${user.user_id.substring(0, 8)}... - Level ${user.level} (${user.total_points} pts, ${user.achievements_earned} achievements)`);
    });
...
async function main() {
  ...
  if (command === 'stats' || command === '--stats') {
    ...
    console.log('Usage:');
    console.log('  node scripts/retroactive-awards.js              # Award all users');
    console.log('  node scripts/retroactive-awards.js <user-id>    # Award specific user');
    console.log('  node scripts/retroactive-awards.js stats        # Show current stats');
    console.log('  node scripts/retroactive-awards.js help         # Show this help');
  } else {
    await awardAllUsers();
  }
